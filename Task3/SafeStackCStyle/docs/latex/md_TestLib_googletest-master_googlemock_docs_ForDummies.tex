...
\begin{DoxyCode}
\textcolor{preprocessor}{#include "gmock/gmock.h"}  \textcolor{comment}{// Brings in Google Mock.}
\textcolor{keyword}{class }MockTurtle : \textcolor{keyword}{public} Turtle \{
 \textcolor{keyword}{public}:
  ...
  MOCK\_METHOD0(PenUp, \textcolor{keywordtype}{void}());
  MOCK\_METHOD0(PenDown, \textcolor{keywordtype}{void}());
  MOCK\_METHOD1(Forward, \textcolor{keywordtype}{void}(\textcolor{keywordtype}{int} distance));
  MOCK\_METHOD1(Turn, \textcolor{keywordtype}{void}(\textcolor{keywordtype}{int} degrees));
  MOCK\_METHOD2(GoTo, \textcolor{keywordtype}{void}(\textcolor{keywordtype}{int} x, \textcolor{keywordtype}{int} y));
  MOCK\_CONST\_METHOD0(GetX, \textcolor{keywordtype}{int}());
  MOCK\_CONST\_METHOD0(GetY, \textcolor{keywordtype}{int}());
\};
\end{DoxyCode}
...
\textcolor{keywordtype}{int} main(\textcolor{keywordtype}{int} argc, \textcolor{keywordtype}{char}** argv) \{
  \textcolor{comment}{// The following line must be executed to initialize Google Mock}
  \textcolor{comment}{// (and Google Test) before running the tests.}
  ::testing::InitGoogleMock(&argc, argv);
  \textcolor{keywordflow}{return} RUN\_ALL\_TESTS();
\}
\end{DoxyCode}
...
{\bfseries Important note\+:} Google \hyperlink{classMock}{Mock} requires expectations to be set {\bfseries before} the mock functions are called, otherwise the behavior is {\bfseries undefined}. In particular, you mustn\textquotesingle{}t interleave {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+C\+A\+L\+L()}s and calls to the mock functions.

This means {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+C\+A\+L\+L()} should be read as expecting that a call will occur {\itshape in the future}, not that a call has occurred. Why does Google \hyperlink{classMock}{Mock} work like that? Well, specifying the expectation beforehand allows Google \hyperlink{classMock}{Mock} to report a violation as soon as it arises, when the context (stack trace, etc) is still available. This makes debugging much easier.
...
If you want to use something other than Google Test (e.\+g. \href{http://sourceforge.net/projects/cppunit/}{\tt Cpp\+Unit} or \href{https://cxxtest.com/}{\tt Cxx\+Test}) as your testing framework, just change the {\ttfamily main()} function in the previous section to\+: 
\begin{DoxyCode}
\textcolor{keywordtype}{int} main(\textcolor{keywordtype}{int} argc, \textcolor{keywordtype}{char}** argv) \{
  \textcolor{comment}{// The following line causes Google Mock to throw an exception on failure,}
  \textcolor{comment}{// which will be interpreted by your testing framework as a test failure.}
  ::testing::GTEST\_FLAG(throw\_on\_failure) = \textcolor{keyword}{true};
  ::testing::InitGoogleMock(&argc, argv);
  ... whatever your \hyperlink{namespacetesting}{testing} framework requires ...
\}
\end{DoxyCode}
...
In Google \hyperlink{classMock}{Mock} we use the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+C\+A\+L\+L()} macro to set an expectation on a mock method. The general syntax is\+:
...
\begin{DoxyCode}
\textcolor{keywordtype}{int} n = 100;
EXPECT\_CALL(turtle, GetX())
.Times(4)
