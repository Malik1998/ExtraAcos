...
namespace FullFileReader {
    ...
    size_t changeSlashesToNulles(char *text, char *** indexes) {
        ...
        (*indexes) = new char*[countOfLines + 1]; // may-be
        size_t k = 0;
        i = 0;
        countOfLines = 0;
        while (text[i] != '\0') {
            if (text[i] == '\n') {
                (*indexes)[countOfLines++] = (text + k);
                k = i + 1;
                text[i] = '\0';
            }
            ++i;
    ...
    void outputInFile(char ** indexes, const char* FileName, size_t countOfLines, int typeOfWriting) {
        int fileDescriptor = open(FileName, typeOfWriting, 0666);
        for (size_t i = 0; i < countOfLines - 1; ++i) {
            write(fileDescriptor, indexes[i], strlen(indexes[i]));
