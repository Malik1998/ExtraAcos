...
\textcolor{keyword}{class }MockFoo : \textcolor{keyword}{public} Foo \{
  ...
  MOCK\_METHOD2(Process, \textcolor{keywordtype}{bool}(Bar elem, \textcolor{keywordtype}{int} count));
\};
\end{DoxyCode}
...
\end{DoxyEnumerate}
\begin{DoxyEnumerate}
\item an array passed as a pointer and a count (e.\+g. in {\ttfamily Bar(const T$\ast$ buffer, int len)} -- see \href{#multiargument-matchers}{\tt Multi-\/argument Matchers}).
\end{DoxyEnumerate}
...
\end{longtabu}
\subsection*{Matching the \hyperlink{classResult}{Result} of a Function or Functor}
