...
TEST(BarTest, DoesThis) \{
  MockFoo foo;                                    \textcolor{comment}{// #2}

  ON\_CALL(foo, GetSize())                         \textcolor{comment}{// #3}
      .WillByDefault(Return(1));
  \textcolor{comment}{// ... other default actions ...}

  EXPECT\_CALL(foo, Describe(5))                   \textcolor{comment}{// #4}
      .Times(3)
      .WillRepeatedly(Return(\textcolor{stringliteral}{"Category 5"}));
  \textcolor{comment}{// ... other expectations ...}

  EXPECT\_EQ(\textcolor{stringliteral}{"good"}, MyProductionFunction(&foo));  \textcolor{comment}{// #5}
\}                                                 \textcolor{comment}{// #6}
\end{DoxyCode}
...
\rowcolor{\tableheadbgcolor}\textbf{ Matcher }&\textbf{ Description  }\\\cline{1-2}
\endhead
{\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+T\+H\+A\+T(value, matcher)} &Asserts that {\ttfamily value} matches {\ttfamily matcher}. \\\cline{1-2}
...
\rowcolor{\tableheadbgcolor}\textbf{ Matcher }&\textbf{ Description  }\\\cline{1-2}
\endhead
{\ttfamily Container\+Eq(container_)} &The same as {\ttfamily Eq(container_)} except that the failure message also includes which elements are in one container_ but not the other. \\\cline{1-2}
...
{\ttfamily Each(e)} &{\ttfamily argument} is a container_ where {\itshape every} element matches {\ttfamily e}, which can be either a value or a matcher. \\\cline{1-2}
...
{\ttfamily Elements\+Are\+Array(\{ e0, e1, ..., en \})}, {\ttfamily Elements\+Are\+Array(array)}, or {\ttfamily Elements\+Are\+Array(array, count_)} &The same as {\ttfamily Elements\+Are()} except that the expected element values/matchers come from an initializer list, S\+T\+L-\/style container_, or C-\/style array. \\\cline{1-2}
{\ttfamily Is\+Empty()} &{\ttfamily argument} is an empty container_ ({\ttfamily container_.\+empty()}). \\\cline{1-2}
...
{\ttfamily Size\+Is(m)} &{\ttfamily argument} is a container_ whose size matches {\ttfamily m}. E.\+g. {\ttfamily Size\+Is(2)} or {\ttfamily Size\+Is(\+Lt(2))}. \\\cline{1-2}
...
{\ttfamily Unordered\+Elements\+Are\+Array(\{ e0, e1, ..., en \})}, {\ttfamily Unordered\+Elements\+Are\+Array(array)}, or {\ttfamily Unordered\+Elements\+Are\+Array(array, count_)} &The same as {\ttfamily Unordered\+Elements\+Are()} except that the expected element values/matchers come from an initializer list, S\+T\+L-\/style container_, or C-\/style array. \\\cline{1-2}
{\ttfamily When\+Sorted(m)} &When {\ttfamily argument} is sorted using the {\ttfamily $<$} operator, it matches container_ matcher {\ttfamily m}. E.\+g. {\ttfamily When\+Sorted(\+Elements\+Are(1, 2, 3))} verifies that {\ttfamily argument} contains elements {\ttfamily 1}, {\ttfamily 2}, and {\ttfamily 3}, ignoring order. \\\cline{1-2}
...
\rowcolor{\tableheadbgcolor}\textbf{ Matcher }&\textbf{ Description  }\\\cline{1-2}
\endhead
{\ttfamily A\+S\+S\+E\+R\+T\+\_\+\+T\+H\+A\+T(expression, m)}&Generates a \href{../../googletest/docs/primer.md#assertions}{\tt fatal failure} if the value of {\ttfamily expression} doesn\textquotesingle{}t match matcher {\ttfamily m}. \\\cline{1-2}
